...
export function App() {
  ...
  useEffect(() => {
    if (isDebugEnabled()) {
      debugWarn('[App] Tab restore useEffect triggered:', {
    ...
    if (projects.length > 0) {
      ...
      if (openProjectIds.length === 0) {
        ...
        debugWarn('[App] No tabs persisted, opening project:', projectToOpen);
        ...
        if (projects.some(p => p.id === projectToOpen)) {
          ...
          debugWarn('[App] Project not found, falling back to first project:', projects[0].id);
          openProjectTab(projects[0].id);
          setActiveProject(projects[0].id);
        }
        return;
      }
      debugWarn('[App] Tabs already persisted, checking active project');
      ...
      if (activeProjectId && !openProjectIds.includes(activeProjectId)) {
        debugWarn('[App] Active project has no tab, opening:', activeProjectId);
      ...
      // If there's a selected project but no active project, make it active
        ...
        debugWarn('[App] No active project, using selected:', selectedProjectId);
        setActiveProject(selectedProjectId);
        openProjectTab(selectedProjectId);
      } else {
        debugWarn('[App] Tab state is valid, no action needed');
      }
    }
  ...
  useEffect(() => {
    ...
    const cleanupDownloaded = window.electronAPI.onAppUpdateDownloaded(() => {
      debugLog('[App] Update downloaded, opening settings to updates section');
  ...
  const handleOpenInbuiltTerminal = (_id: string, cwd: string) => {
    ...
    debugLog('[App] Opening inbuilt terminal:', { cwd });
    ...
    if (!terminal) {
      ...
      debugWarn('[App] Terminal added to store:', terminal.id);
    }
  };
  ...
  const handleInitialize = async () => {
    ...
    debugWarn('[InitDialog] Starting initialization for project:', projectId);
    setIsInitializing(true);
    setInitSuccess(false);
    setInitError(null); // Clear any previous errors
    try {
      const result = await initializeProject(projectId);
      debugWarn('[InitDialog] Initialization result:', result);

      if (result?.success) {
        debugWarn('[InitDialog] Initialization successful, closing dialog');
        // Get the updated project from store
        const updatedProject = useProjectStore.getState().projects.find(p => p.id === projectId);
        debugWarn('[InitDialog] Updated project:', updatedProject);
        ...
        debugWarn('[InitDialog] Initialization failed, showing error');
  ...
  const handleSkipInit = () => {
    debugWarn('[InitDialog] User skipped initialization');
    if (pendingProject) {
    ...
    <ViewStateProvider>
      ...
      <div className="flex h-screen bg-background">
        ...
        <Dialog open={showInitDialog} onOpenChange={(open) => {
          debugWarn('[InitDialog] onOpenChange called', { open, pendingProject: !!pendingProject, isInitializing, initSuccess });
